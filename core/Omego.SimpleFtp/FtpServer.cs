namespace Omego.SimpleFtp
{
    ...
    public class FtpServer : IFtpServer
    {
        ...
        public void Start()
        {
            ...
            if (!fileSystem.Directory.Exists(fileSystem.Path.GetDirectoryName(tempFilePath))) fileSystem.Directory.CreateDirectory(fileSystem.Path.GetDirectoryName(tempFilePath));

            fileSystem.File.WriteAllBytes(tempFilePath, Assets.ftpdmin);

            process = operatingSystem.StartProcess(
                tempFilePath,
                $"-p {Configuration.Port} \"{Configuration.HomeDirectory}\"");
        }
        ...
        {
            foreach (var file in fileSystem.Directory.GetFiles(Path.Combine(Configuration.HomeDirectory, relativePath))) yield return Path.GetFileName(file);
        }
    }
}